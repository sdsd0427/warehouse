...
<c:set var="imporMailList" value="${dataMap.imporMailList }" />
				...
				<section class="content-header">
					<div class="row md-2">
						<div class="col" style="display: inline; width: 200px">
							<h3 style="width: 200px; display: inline;">중요메일함</h3>
						...
						<div class="table-responsive mailbox-messages">
							<table class="table table-hover text-nowrap">
								...
								<tbody>
									...
									<c:forEach items="${imporMailList }" var="mail">
										<c:forEach items="${mail.mtoList }" var="mto">
											...
											<td>
												<c:if test="${mail.name eq loginUser.name }">
													<button type="button" class="btn btn-sm checkbox-toggle imporMail" onclick="starClick('updateImporCancel.do', '${mail.mailNo}')" >
														<i class="fas fa-star text-warning"></i>
													</button>
												</c:if>
												<c:if test="${mail.name ne loginUser.name }">
													<button type="button" class="btn btn-sm checkbox-toggle imporMail" onclick="starClick('updateMtoImporCancel.do', '${mail.mailNo}')" >
														<i class="fas fa-star text-warning"></i>
													</button>
												</c:if>
											</td>
											<td>${mail.name }</td>
											<td style="text-align: left; cursor: pointer;" onclick="goDetail('mail/detailGo.do?mailNo=${mail.mailNo }&test=importantMail&from=${menu.mcode}&eno=${loginUser.eno}','${menu.upcode }')">
	      ...
	      Swal.fire(
	        '휴지통으로 이동되었습니다.'
	    	  ...
	    	  $("input[type='checkbox']:checked").each(function(index){
	  			...
	  			location.href='<%=request.getContextPath()%>/mail/trashUpdate.do?mailNo='+mailNo+'&mCode='+'${menu.mcode}&move=impor';
	  		})
	      })
	    }
	 })
		      ...
		      Swal.fire(
		        '메일을 읽음처리 하였습니다.'
		    	  ...
		    	  $("input[type='checkbox']:checked").each(function(index){
						var mailNo = $(this).next().val();
//		 				alert(mailNo);
						location.href='mtoCheck.do?mailNo='+mailNo+'&eno='+ ${loginUser.eno} +'&mCode='+'${menu.mcode}' +'&move=importantMail';
					});
		      })
		    }
		 })
