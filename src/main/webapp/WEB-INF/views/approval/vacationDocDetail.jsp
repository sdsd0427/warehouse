				...
				<tr>
							...
							<tbody id="tableBot">
								<tr>
									...
									<td style="background: rgb(255, 255, 255); padding: 5px; border: 1px solid black; border-image: none; text-align: left; color: rgb(0, 0, 0); font-size: 12px; font-weight: normal; vertical-align: middle;">
										<span data-select-option="true" id="loginName">${signDoc.name }</span>
								...
								<tr>
									...
									<td style="background: rgb(255, 255, 255); padding: 5px; border: 1px solid black; border-image: none; text-align: left; color: rgb(0, 0, 0); font-size: 12px; font-weight: normal; vertical-align: middle;">
										<span data-select-option="true" id="deptName">${signDoc.dname }</span>
								...
								<tr>
									...
									<td style="background: rgb(255, 255, 255); padding: 5px; border: 1px solid black; border-image: none; text-align: left; color: rgb(0, 0, 0); font-size: 12px; font-weight: normal; vertical-align: middle;">
										<span data-select-option="true">
											<fmt:formatDate value="${signDoc.ddate }" pattern="yyyy-MM-dd" />
										</span>
									</td>
								</tr>
								<tr>
									...
									<td style="background: rgb(255, 255, 255); padding: 5px; border: 1px solid black; border-image: none; text-align: left; color: rgb(0, 0, 0); font-size: 12px; font-weight: normal; vertical-align: middle;">
										<span data-select-option="true" id="signDOCNO">${signDoc.signNo }</span>
									</td>
								</tr>
							</tbody>
						</table>
					...
					<td style=" background: white;text-align: right; padding: 0px !important; border: currentColor; border-image: none; color: black; font-size: 12px; font-weight: normal; vertical-align: top;">
							...
							<span class="sign_member_wrap" id="activity_14264">
								<span class="sign_member">
									<span class="sign_rank_wrap">
										<span id="ppsloginname"class="sign_rank">${signDoc.ppsname }
											<input type="hidden" name="ppsname" value="">
										</span>
									</span>
									<span class="sign_wrap">
										<span class="sign_stamp">
											<img src="<%=request.getContextPath()%>/resources/images/stamp_approved.png">
										</span>
										<span id="sign_name"><strong>${signDoc.name }</strong></span>
									</span>
									<span class="sign_date_wrap">
										<span class="sign_date " id="date_14264"><fmt:formatDate value="${signDoc.ddate }" pattern="yyyy-MM-dd" /></span>
									</span>
								</span>
							</span>
						...
						<c:forEach items="${signDoc.signerList }" var="signer">
							<span class="sign_type1_inline" data-signer="${signer.eno}" data-group-seq="1" data-group-name="승인" data-group-max-count="7" data-group-type="type1" data-is-reception="">
								<span class="sign_tit_wrap">
								 	...
								 	<span class="sign_member">
								 		...
								 		<span class="sign_wrap">
								 			...
								 			<c:if test="${signer.signCheck eq 3 }">
								 				<span class="sign_stamp" style="padding-top: 19px;">
										...
										<span class="sign_date_wrap">
											<span class="sign_date " id="date_14265">
												<c:if test="${signer.signCheck eq 2 }">
									 				<fmt:formatDate value="${signer.signDate }" pattern="yyyy-MM-dd" />
									 			</c:if>
												<c:if test="${signer.signCheck eq 3 }">
									 				<span style="color: red;"><fmt:formatDate value="${signer.signDate }" pattern="yyyy-MM-dd" /></span>
									 			</c:if>
											</span>
										</span>
									</span>
...
<br>
			...
			<table class="__se_tbl"
				style="background: white; border: 1px solid black; border-image: none;  color: black; font-family: malgun gothic, dotum, arial, tahoma; font-size: 12px; margin-top: 0px; border-collapse: collapse !important;">
				<tbody>
					<tr>
					...
					<tr>
						...
						<td
							...
							data-maxlength="" data-width="" data-defaultstr="" disabled="disabled"
							data-editable="false" name="vacStart" data-value="" value="<fmt:formatDate value="${signDoc.vacStart }" pattern="yyyy-MM-dd" />" onclick="DateStart()">&nbsp;~&nbsp;

							<input type="date"
							class="ipt_editor ipt_editor_date hasDatepicker"
							data-dsl="{{calendar:vacEnd}}" id="vacEnd" data-id="vacEnd"
							data-name="vacEnd" data-require="false" data-maxlength=""
							data-width="" data-defaultstr="" data-editable="false" disabled="disabled"
							name="vacEnd" data-value="" value="<fmt:formatDate value="${signDoc.vacEnd }" pattern="yyyy-MM-dd" />"></td>
					</tr>
					<tr>
						<td
					...
					<tr>
						...
						<td
							...
							<div style="border: 1px solid lightgray; border-radius: 3px; padding: 4px; width: 100%; margin-top: 9px; height: 200px; overflow: scroll;">${signDoc.signContent }</div>
						</td>
					</tr>
					<tr>
						<td
