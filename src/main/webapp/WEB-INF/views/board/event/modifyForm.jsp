	...
	<section class="content container-fluid">
		<div class="card card-navy card-outline">
			<div class="card-header">
				<h3 class="card-title" style="padding-top: 8px;">경조사게시판 수정</h3>
				<div class="col-lg-12">
			...
			<div class="card-body">
				<form enctype="multipart/form-data" role="form" method="post" action="eventModify.do" name="modifyForm">
				<input type="hidden" name="boardNo" value="${board.boardNo }" />
					<div class="form-group row">
						<label for="title" class="col-2">경조 구분</label>
						<select class="form-control col-3" id="eventcate" name="eventcate" disabled>
							<option value="1" ${board.eventcate eq 1 ? 'selected':'' }>결혼</option>
							<option value="2" ${board.eventcate eq 2 ? 'selected':'' }>부고</option>
						</select>
					</div>
					<div class="form-group row">
						<label for="title" class="col-2">게시글 만료일</label>
						<input type="date" id="enddate" value="${board.enddate }" name="enddate" class="form-control form-control col-3">
					</div>
					<div class="form-group row">
