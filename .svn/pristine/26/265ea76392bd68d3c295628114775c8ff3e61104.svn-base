			...
			<div class="container-fluid">
				<div class="row md-2">
					...
					<div style="display: inline; margin-left: 10px; margin-top: 10px;">
						<span  class="text-muted"  style="padding-right: 535px;">전자결재 > 임시문서함</span>
					</div>
				</div>
			</div>
		</section>
			...
			<div class="col-12">
				<div style="display: flex; justify-content: space-between; float: right;">
					<div class="input-group input-group-sm row approve-search" style="display: flex; margin-top: 20px;">
						<select class="form-control col-3" name="signForm" id="signForm" onchange="list_go('tempList.do',1,0);" style="width: 30%;">
						...
						<span class="input-group-append">
							<button class="btn btn-dark" type="button" data-card-widget="search" onclick="list_go('tempList.do',1,0);">
			...
			<div class="card col-12" style="margin-top: 15px; padding: 0;">
				<div class="card-header bg-navy p-0 pt-1 border-bottom-0">
					<ul class="nav nav-tabs" id="custom-tabs-three-tab" role="tablist">
						<li class="nav-item">
							<a class="nav-link active" id="custom-tabs-three-home-tab" data-toggle="pill" role="tab" aria-controls="custom-tabs-three-home" aria-selected="true" onclick="list_go('tempList.do',1,0);">전체</a>
						</li>
					</ul>
				</div>
					...
					<table class="table text-nowrap projects">
						<thead>
							<tr>
								...
								<th style="width: 10%; padding: 0.5rem;" class="fieldItem" id="제목">제목</th>
								<th style="width: 5%; padding: 0.5rem;" class="fieldItem" id="첨부">첨부</th>
								<th style="width: 10%; padding: 0.5rem;" class="fieldItem" id="기안부서">기안부서</th>
							</tr>
						</thead>
						<tbody>
							...
							<c:forEach items="${tempList }" var="temp">
								<tr onclick="goDetail('approval/detailGo.do?signNo=${temp.signNo }','${menu.mcode }')">
									...
									<td class="field-content" data-field="결재양식">${temp.formText}</td>
									<td class="field-content" data-field="긴급">
										<c:if test="${temp.emergency eq '1'}">
											<span class="material-symbols-outlined">e911_emergency </span>
										</c:if>
									</td>
									<td class="field-content" data-field="제목" style="text-align: left; padding-left: 58px; max-width: 130px; overflow: hidden; white-space: nowrap; text-overflow: ellipsis;">${temp.title}</td>
									<td class="field-content" data-field="첨부">
										<c:if test="${!empty temp.attachList }">
											<i class="fas fa-paperclip"></i>
										</c:if>
									</td>
									<td class="field-content" data-field="기안부서">${temp.dname}</td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
...
<%-- 					<%@ include file="/WEB-INF/views/common/pagination.jsp" %> --%>
					...
					<ul class="pagination justify-content-center m-0">
						<li class="page-item">
							<a class="page-link" href="javascript:list_go('tempList.do',1,${dataMap.state })">
								<i class="fas fa-angle-double-left"></i>
							</a>
						</li>
						<li class="page-item">
							<a class="page-link" href="javascript:list_go('tempList.do',${pageMaker.cri.page - 1 lt 0 ? 1 : pageMaker.cri.page - 1 },${dataMap.state })">
								<i class="fas fa-angle-left"></i>
							</a>
						</li>
						<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }" var="page">
							<li class="page-item ${page eq pageMaker.cri.page ? 'active' : ''}">
								<a class="page-link" href="javascript:list_go('tempList.do',${page },${dataMap.state })">
								${page }
								</a>
							</li>
						</c:forEach>
						<li class="page-item">
							<a class="page-link" href="javascript:list_go('tempList.do',${pageMaker.cri.page+1 gt pageMaker.realEndPage ? pageMaker.realEndPage : pageMaker.cri.page+1},${dataMap.state })">
								<i class="fas fa-angle-right"></i>
							</a>
						</li>
						<li class="page-item">
							<a class="page-link" href="javascript:list_go('tempList.do',${pageMaker.realEndPage },${dataMap.state })">
