...
<c:set var="toMeMailList" value="${dataMap.toMeMailList }" />
						...
						<div class="table-responsive mailbox-messages">
							<table class="table table-hover text-nowrap">
								...
								<tbody>
									...
									<c:forEach items="${toMeMailList }" var="mail">
										<c:forEach items="${mail.mtoList }" var="mto">
											...
											<td>${mail.name }</td>
											<td style="text-align: left; cursor: pointer;" onclick="goDetail('mail/detailGo.do?mailNo=${mail.mailNo }&test=toMeMail&from=${menu.mcode}&eno=${loginUser.eno}','${menu.upcode }')">
	      ...
	      Swal.fire(
	        '휴지통으로 이동되었습니다.'
	    	  ...
	    	  $("input[type='checkbox']:checked").each(function(index){
	  			...
	  			location.href='<%=request.getContextPath()%>/mail/trashUpdate.do?mailNo='+mailNo+'&mCode='+'${menu.mcode}&move=tome';
	  		})
	      })
	    }
	 })
		      ...
		      Swal.fire(
		        '메일을 읽음처리 하였습니다.'
		    	  ...
		    	  $("input[type='checkbox']:checked").each(function(index){
						var mailNo = $(this).next().val();
//						alert(mailNo);
						location.href='mtoCheck.do?mailNo='+mailNo+'&eno='+ ${loginUser.eno} +'&mCode='+'${menu.mcode}' +'&move=toMeMail';
					});
		      })
		    }
		 })
