...
<li class="paginate_button page-item {{signActive this}} ">
	<a href="javascript:getPage('<%=request.getContextPath()%>/commuReplies/${commuPost.cpostNo}/{{this}}',{{this}});" aria-controls="example2" data-dt-idx="1" tabindex="0" class="page-link">
		{{this}}
	</a>
</li>
{{/each}}
...
window.onload=function(){
	getPage("<%=request.getContextPath()%>/commuReplies/${commuPost.cpostNo}/"+replyPage);
}
...
// 			"name":"${manual.name}",
	...
	$.ajax({
		url:"<%=request.getContextPath()%>/commuReplies",
		type:"post",
		...
		success:function(data){
			...
			Swal.fire({
		      ...
		      confirmButtonColor: '#3085d6',
				...
				getPage("<%=request.getContextPath()%>/commuReplies/${commuPost.cpostNo}/"+data); //리스트 출력
				$('#newReplyText').val("");
		    });
		},
...
function replyModify_go(){
	...
	$.ajax({
		url:"<%=request.getContextPath()%>/commuReplies/"+rno,
		type:"PUT",
...
function replyRemove_go(rno){
	...
	$.ajax({
		url:"<%=request.getContextPath()%>/commuReplies/${commuPost.cpostNo}/"+rno+"/"+replyPage,
		type:"DELETE",
		headers:{
		...
		success:function(page){
			...
			Swal.fire({
			      ...
			      confirmButtonColor: '#3085d6',
			    	...
			    	getPage("<%=request.getContextPath()%>/commuReplies/${commuPost.cpostNo}/"+page);
					replyPage=page;
			    });
		},
		error:function(error){
